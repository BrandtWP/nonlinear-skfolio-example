...
ql_date_to_dt_date = lambda ql_date: dt.date(ql_date.year(), ql_date.month(), ql_date.dayOfMonth())

class QLMarketContext(MarketContext):
    quotes: Dict[str, ql.SimpleQuote] = {}
    relinkable_handles: Dict[str,  "ql.RelinkableHandle"] = {}

    def __init__(self, date=None, ql_env: Dict[str, "ql.QuoteHandle | ql.RelinkableHandle"] | None = None,
                 **kwargs):
        ...
        for k, v in ql_env.items():
            if isinstance(v, ql.SimpleQuote):
                ...
                raise ValueError(f"Unsupported QuantLib object for key '{k}': {type(v)}")
    ...
    def __sklearn_clone__(self):
        ...
        for k, v in self.quotes.items():
            new_ql_env[k] = v  # SimpleQuotes are mutable but we assume user handles cloning if needed
        for k, v in self.relinkable_handles.items():
            new_ql_env[k] = v  # RelinkableHandles are mutable but we assume user handles cloning if needed
        return QLMarketContext(
            date=self.date,
            ql_env=new_ql_env,
            **self.data
        )
